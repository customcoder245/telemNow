...
function get_affilate_token_key(){
	
		 $master_token =get_option('tel_user_name').':'.get_option('tel_Password');
		$curl = curl_init();
			$tel_url=get_option('eurl');
			curl_setopt_array($curl, array(
			  CURLOPT_URL => $tel_url.'/auth/client',
			  ...
			  CURLOPT_CUSTOMREQUEST => 'POST',
			  CURLOPT_HTTPHEADER => array(
				'Authorization: Basic '.base64_encode($master_token)
			  ),
			));
...
foreach ( $order->get_items() as $item_id => $item ) {
	...
	$curl = curl_init();
    $keys=get_option('affiliate_token');
	$tel_url=get_option('eurl');
curl_setopt_array($curl, array(
